...
Since this proxy is publicly accessible, an attacker could bypass your app and acess the OpenAI API with your private key directly at `https://functionroute.yourcloud.com/`.

## Mistake # 3: Using an Authenticated Proxy without Permissions

Putting your OpenAI API key as a secret in a hosted server or cloud function and adding authentication so only your users can call the proxy endpoint is quite common and still much better than the setups laid out in the previous two mistakes. However, it is *still* unsafe. Let's take the following frontend code:
...
1. There are no limits to how many times a given user can call the OpenAI API proxy. Any user can call this API proxy up to the default account [rate limits](https://platform.openai.com/docs/guides/rate-limits) for the Open AI API associated with the private API key stored in the backend by simply getting their JWT and calling the proxy at `https://functionroute.yourcloud.com/v1/`. The JWTs, or authentication tokens, typically expire within an hour. That means that someone can generate a token from your client-side code and (mis)use it until it expires. Then rinse and repeat.
2. OpenAI, and other LLM APIs, have endpoints that allow uploading [files](https://platform.openai.com/docs/api-reference/files) for augmented analysis and creating [threads](https://platform.openai.com/docs/api-reference/threads) with chat history for autonomous assistatns. The proxy layed out above does not provide any permissions or access control over which users uploaded or created which private resources. This means that any user with just their JWT can exploit this security vulnerability and see all the files and conversations ever created with the backend's OpenAI API key. This will cause infringements on the data and privacy of end users. The following frontend code will call the OpenAI API [list files](https://platform.openai.com/docs/api-reference/files/list) end point which will return all the files ever uploaded regardless of the user that uploaded it:
...
From your perspective, there’s “no backend” to manage—no servers, no DevOps, no complicated configurations. But under the hood, Backmesh is a secure middleman between your front end and OpenAI API. Read more about Backmesh's security considerations [here](/docs/security) or check our the open source code on [Github](https://github.com/backmesh/backmesh).